-- ============================================================
-- Migration 048: Fix date format in trend RPCs
--
-- generate_series(date, date, interval) in PostgreSQL implicitly
-- casts to timestamp, so ::text gives "YYYY-MM-DD HH:MI:SS".
-- Use to_char(..., 'YYYY-MM-DD') to always return clean date strings.
-- ============================================================
...
BEGIN
  RETURN (
    ...
    FROM (
      SELECT
        to_char(g.date::date, 'YYYY-MM-DD')                                          AS date,
  ...
  RETURN (
    ...
    FROM (
      ...
      LEFT JOIN scheduling.appointments a
        ...
        AND a.scheduled_start_at::date = g.date::date
      GROUP BY g.date::date
    ) d
  );
END;
