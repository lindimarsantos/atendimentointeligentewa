-- ============================================================
-- Migration 040: Improve rpc_n8n_post_interaction
--
-- summary_text  → narrative prose for humans + AI memory
-- open_items_jsonb → [{role, text}] array for "Memória Recente" panel
-- ============================================================
...
BEGIN
  ...
  -- ── 2. Create AI session (required FK for ai_decisions) ─────────────────────
  ...
  -- ── 5. Last 8 messages as [{role, text}] for Memória Recente ────────────────
  ...
  -- ── 6. Check for latest appointment ─────────────────────────────────────────
  ...
  -- ── 7. Build narrative summary (prose readable by humans and AI) ─────────────
  v_summary_text :=
  ...
  v_summary_text := v_summary_text ||
    ' Última resposta da IA: ' || p_reply_text;

  -- ── 8. Facts (structured metadata) ─────────────────────────────────────────
  ...
  IF FOUND THEN
    ...
    -- Upsert customer memories for the booked service
  ...
  -- ── 9. Upsert running conversation summary ──────────────────────────────────
